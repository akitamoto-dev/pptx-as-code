...
1. 同じ利用者の別の環境で、設定されている取得先を尋ねる
   ...
   **尋ねるときは、調べ方も一緒に示す。** 「index-url を教えてください」と URL の例だけでは、初めての人は何を答えればよいか分からない。実行するコマンドと、**どこで実行するか**を添える。次のように尋ねる。

   > 組織のパッケージ取得先が設定されていないか確認したいです。分からない場合は、
   > **同じネットワーク経路の環境**（Docker Desktop をお使いならコンテナではなく Windows のターミナル）
   > で次を実行し、表示された値を教えてください。
   >
   > ```
   > npm config get registry
   ...
   > ```
   >
   > 既定（`https://registry.npmjs.org/`、PyPI）以外が表示されたら、それが組織の取得先です。
   > 何も設定されていなければ「無い」と答えてください。画像生成 MCP 以外を完了させます。
