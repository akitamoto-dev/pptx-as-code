...
1. 同じ利用者の別の環境で、設定されている取得先を尋ねる
   ...
   **尋ねるときは、次の 3 つを必ず含める。** 「index-url を教えてください」と URL の例だけでは、利用者はどの端末のどのターミナルで何をすればよいか分からない。

   **(1) いま自分が動いている環境を名乗る。** `doctor.sh` の見出しに `(コンテナ)` `(WSL)` が出るので、それを使う。利用者は、エージェントがどこで動いているかを知らない。

   **(2) 調べる場所を、環境から決めて名指しする。**

   | いま動いている環境 | 調べる場所 |
   |---|---|
   | コンテナ（Docker Desktop on Windows） | **Windows** のターミナル。コンテナの中や WSL では既定値しか出ない |
   | WSL | **Windows** のターミナル。または WSL から `cat /mnt/c/Users/<Windows のユーザー名>/.npmrc` |
   | Windows ネイティブ | その端末 |
   | Linux / macOS | その端末 |

   **(3) その場所で動くコマンドを示す。** Windows を調べてもらう場合、設定ファイルを直接読む方が確実（npm や Python が Windows 側に入っていなくても読める）。

   > いま**コンテナの中**で動いています。コンテナは Windows と同じネットワーク経路を通るため、
   > 組織の取得先は Windows 側に設定されています。**Windows の PowerShell**（コンテナの
   > ターミナルでも WSL でもなく）で次を実行し、表示された内容を教えてください。
   >
   > ```
   ...
   > ファイルが無いと表示されたら「無い」と答えてください。画像生成 MCP 以外を完了させます。

   **返ってきた出力が、指定した場所のものかを確かめる。** プロンプトやパスが別の環境のもの（`~/dev/...` のような Linux のパス、`$` や `➜` のプロンプト）であれば、Windows ではない。その場合は結論を出さず、**どこで実行してほしいかをもう一度伝えて依頼し直す**。

   **片方しか見つからない場合は、同じホストのもう片方を試す。** 組織のパッケージプロキシは、1 つのホストが npm・PyPI などをパス違いで配っていることが多い。実際、npm だけ `.npmrc` に設定されていて Python 側は未設定、という環境がある。その場合は同じホストの PyPI 用パス（`https://<ホスト>/pypi/simple/` など）に到達できるかを確かめてから使う。

   ```bash
   curl -sS -o /dev/null -w '%{http_code}\n' https://<ホスト>/pypi/simple/fastmcp/
   ```

   組織が用意した同じ取得先なので、別のミラーを使うことにはならない。応答しなければ諦め、利用者に尋ねる。

   **過去の記録に「取得先が無い」とあっても、そのまま信じない。** 調べた環境が違っていた可能性がある（WSL で調べてコンテナに当てはめる、など）。塞がれていると分かった時点で、その環境について改めて尋ね直す。
...
2. 既定（`https://registry.npmjs.org/`、PyPI）以外が設定されていれば、作業する環境にも同じ値を設定する。**この設定変更は利用者の判断が要る**ので、値を見せて確認してから行う
   ...
   **環境変数（`UV_INDEX_URL` など）では足りない。** 画像生成 MCP はエージェントが子プロセスとして起動するため、シェルで設定した値が引き継がれない。上のように設定ファイルへ書く。書いたらエージェントを起動し直す（MCP の定義は起動時に読まれる）。
...
3. **社内の取得先が無い、または分からない場合は、公式の配布元に切り替える。** 推測ではないので、承諾を待たずに進めてよい
   ...
   **画像生成 MCP の依存（fastmcp、openai、azure-identity ほか）には、この切り替えが効かない。** PyPI 以外の配布が無く、公式 GitHub から入れると依存の木が深すぎて現実的でないため。ここだけは組織の取得先が要る。分からないまま進める場合は、**MCP は使えないと伝え、画像生成なしで残りを完了させる**（スライド作成そのものは MCP なしで動く）。取得先が分かったら、ステップ 2 のとおり `~/.config/uv/uv.toml` に書き、**エージェントを起動し直す**。
