...
**`grep` や `Select-String` で絞り込ませない。ファイルをそのまま表示させる。** 絞り込むと 2 つの理由で取りこぼす。

- `.npmrc` の先頭に UTF-8 BOM が付いていることがあり、`^\s*registry` のような行頭の一致が外れる。設定があるのに「無い」と見える
- `Select-String -Path` は、指定したパスの 1 つが存在しないとエラーになる。`pip.ini` が無い環境は珍しくない

表示された内容は自分で読む。`type` や `cat` は存在しないファイルでエラーを出すが、それは「そのファイルは無い」という答えとして扱ってよい。
