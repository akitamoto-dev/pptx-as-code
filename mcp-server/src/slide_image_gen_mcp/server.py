...
- **`output_dir` と `reference_image_path` は絶対パスで渡す。** このサーバーのカレント
  ...
  保存先は、資料を作っている場合はその作業フォルダーの中、画像だけを作る場合は利用者が
  開いているプロジェクト（VS Code のワークスペース、またはエージェントを起動したフォルダー）の
  中を絶対パスで指定する。省略するとカレントディレクトリ基準の `./output` になり、クライアント
  によってはプラグインの導入先（更新のたびに差し替わる）に保存される。
...
def generate(subject: str) -> str:
    ...
    return (
        ...
        "- 保存先は、利用者が開いているプロジェクトの中を絶対パスで指定する。"
        "会話で別の場所が決まっていれば、そこに従う\n"
        "- タイトル・見出し・図中のラベルを短い日本語で含める。文字を抜いた図版だけにしない\n"
        "- 書式スキル（blue-format など）を併用している場合は、その theme.json の "
...
def generate_slide_image(
    ...
    output_dir: Annotated[
        ...
        Field(
            description=(
                "保存先ディレクトリ（**絶対パス**）。資料を作っている場合はその作業フォルダーの中、"
                "画像だけを作る場合は利用者が開いているプロジェクトの中を指定する。"
